...
- Every temporal or weighted formula must carry a validated `factor_calculation_contract/v1` from Stage 1 through the implementation artifact. Preserve literal source parameters with value/unit/role; declare natural-month, exchange-session, or security-observation windows explicitly; declare weighted means as `sum(weighted values) / sum(weights)`; and keep any observation-count conversion separate with provenance.
- Stage 3 must build formula-input validation with `DataFrameValidationRequest.from_spec(...)`. Load enough pre-sample calculation history for the first scored signal; insufficient history is blocking, not a warm-up warning. Never start rolling calculation at the paper score-sample start.
- Formula tests must record every semantic assertion ID required by the calculation contract. In particular, use asymmetric weights to prove the weighted denominator, test literal parameter units independently from window observation counts, test natural-month boundaries against unequal month lengths, and test the first scoring date with retained prehistory.
